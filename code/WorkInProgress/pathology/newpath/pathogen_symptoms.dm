...
datum/pathogeneffects/malevolent/junglefever
	...
	disease_act(var/mob/M as mob, var/datum/pathogen/origin)
		...
		switch (origin.stage)
			if (1 to 3)
				...
				if ((prob(origin.stage - 1)) && !(M.reagents.has_reagent("potassium")))
					M.show_message("<span style=\"color:red\">You could really go for a banana right now!</span>")
			if (4 to 5)
				if ((prob(origin.stage*2)) && !(M.reagents.has_reagent("potassium")))
					M.show_message("<span style=\"color:red\">You feel a sharp pang of hunger for bananas!</span>")
				if (prob(13))
					if (prob((origin.stage-3)*15))
						M.show_message("<span style=\"color:red\">You feel your hair begin to lengthen.</span>")
						M.reagents.add_reagent("hairgrownium", 20)
	...
	may_react_to()
		return "The pathogen appears to be in some sort of electrolyte withdrawal"

	react_to(var/R, var/zoom)
		if (R == "potassium")
			return "The potassium seems to have satiated the pathogen's cravings!"
